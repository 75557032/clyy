...
void QFileDialogComboBox::setRootList(const QStringList &paths)
{
    m_rootlist = paths;
}
...
void QFileDialog::setRootList(const QStringList &paths)
{
    Q_D(QFileDialog);
    d->qFileDialogUi->lookInCombo->setRootList(paths);
}

QStringList QFileDialog::rootList() const
{
    Q_D(const QFileDialog);
    return d->qFileDialogUi->lookInCombo->rootList();
}
...
void QFileDialogComboBox::showPopup()
{
    ...
    if (m_rootlist.count() <= 0) {
        ...
        for (int i = 0; i < m_rootlist.count(); ++i) {
            QUrl path = QUrl::fromLocalFile(m_rootlist.at(i));
